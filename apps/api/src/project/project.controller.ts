...
export class ProjectController {
  ...
  async create(@WorkspaceId() ws: string, @Body() dto: CreateProjectDto) { return { value: await this.svc.create(ws, dto) }; }
  @Patch(':id') @RequirePermission('resources.write')
  async update(@WorkspaceId() ws: string, @Param('id') id: string, @Body() dto: UpdateProjectDto) { return { value: await this.svc.update(ws, id, dto) }; }
  @Delete(':id') @RequirePermission('resources.write')

...
export class GenerationJobController {
  ...
  @Post() async create(@WorkspaceId() ws: string, @Body() dto: CreateJobDto) { return { value: await this.svc.create(ws, dto) }; }
  @Patch(':id/status') async updateStatus(@WorkspaceId() ws: string, @Param('id') id: string, @Body() dto: UpdateStatusDto) { return { value: await this.svc.updateStatus(ws, id, dto) }; }
}
